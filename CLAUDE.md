...
- `art.py` — sprites 100 % procéduraux. Cache global `ART`. `PLAYER_COLORS` : 8 factions (pid 0..7) + zombies (pid 8). `Terrain` : fond de carte en chunks de 256×256 px générés à la demande (cache LRU borné ≈ 42 Mo) — jamais de surface plein monde, quelle que soit la taille de la carte.
...
6. **Penser aux ordres de grandeur.** La carte « géante » 512×256 cases créait une surface de fond de 16384×8192 px ≈ **537 Mo**. Avant d'ajouter une taille/quantité, calculer la mémoire et le coût CPU (fond, brouillard, A*). (Résolu en v0.15 : fond chunké `art.Terrain`, ces tailles sont depuis rétablies à la demande de Bastien.)
